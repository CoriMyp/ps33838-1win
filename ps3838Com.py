...
class Ps3838Com:
    ...
    async def get_totals(self, i, totals, add_to_type_name, add_to_type):
        if i == 0:
            ...
            line = f"{totals[1]}"
            price = float(totals[2]) if totals[2] != '' else 0.0
        elif i == 1:
            type_ = f'{add_to_type}U'
            line = f"{totals[1]}"
            price = float(totals[3]) if totals[3] != '' else 0.0
        scanner_format_bet = {
...
#        return random.choice(proxies)
    ...
    async def run(self, matches):
        ...
        markets = {}
        for a in all_sport_markets:
            if a:
                markets[list(a)[0]] = a[list(a)[0]]
